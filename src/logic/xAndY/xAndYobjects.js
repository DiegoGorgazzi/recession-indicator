...
export const xAndYobjects = (
  ...
  let startIndex = array.findIndex(
    object =>
      object[xPropDateFromArray].getTime() === datedUserStartDate.getTime()
  );
  let endIndex = array.findIndex(
    object =>
      object[xPropDateFromArray].getTime() === datedUserEndDate.getTime()
  );
  ...
  if (startIndex === -1) {
    array.map((object, index) => {
      if (
        array[0][xPropDateFromArray].getTime() > datedUserStartDate.getTime()
      ) {
        startIndex = 0;
      } else if (
        object[xPropDateFromArray].getTime() > datedUserStartDate.getTime() &&
        array[index - 1][xPropDateFromArray].getTime() <
          datedUserStartDate.getTime()
      ) {
        startIndex = index - 1;
      }
      return startIndex;
  ...
  if (endIndex === -1) {
    array.map((object, index) => {
      if (datedUserEndDate.getTime() < array[0][xPropDateFromArray].getTime()) {
        ...
        object[xPropDateFromArray].getTime() > datedUserEndDate.getTime() &&
        array[index - 1][xPropDateFromArray].getTime() <
          datedUserEndDate.getTime()
      ) {
        endIndex = index;
      } else if (
        array[array.length - 1][xPropDateFromArray].getTime() <
        datedUserEndDate.getTime()
      ) {
